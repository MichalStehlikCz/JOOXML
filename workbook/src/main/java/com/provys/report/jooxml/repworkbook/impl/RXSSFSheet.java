...
public class RXSSFSheet implements Sheet, RepSheet
{
    ...
    private final TreeMap<Integer, RXSSFRow> _rows = new TreeMap<>();
    private final SheetDataWriter _writer;
    private int _randomAccessWindowSize = RXSSFWorkbook.DEFAULT_WINDOW_SIZE;
    private final AutoSizeColumnTracker _autoSizeColumnTracker;
    private int outlineLevelRow;
    private int lastFlushedRowNumber = -1;
    private boolean allFlushed;

    public RXSSFSheet(RXSSFWorkbook workbook, XSSFSheet xSheet) throws IOException {
        _workbook = workbook;
        _sh = xSheet;
...
//start of interface implementation
     ...
     */
    @Override
    public RXSSFRow createRow(int rownum)
    {
    ...
    public void removeRow(Row row)
    {
        ...
        for(Iterator<Map.Entry<Integer, RXSSFRow>> iter = _rows.entrySet().iterator(); iter.hasNext();) {
            Map.Entry<Integer, RXSSFRow> entry = iter.next();
    ...
    /**
     * Copies row from template (base sheet) to current rows
     */
    public void copyRowFromTemplate(int templateRownum, int rownum) {
        Row templateRow = _sh.getRow(templateRownum);
        if (templateRow != null) {
            // create new row
            RXSSFRow row = createRow(rownum);
            // go through cells
            for (Cell templateCell : templateRow) {
                // and copy them to our sheet
                RXSSFCell cell = row.createCell(templateCell.getColumnIndex());
                cell.copyFromTemplate(templateCell);
            }
        }
    }

    /**
     * Copies content of several rows from template (base sheet) to current rows.
     * First row is placed on specified position; this position must be after last row in this sheet, otherwise
     * throws exception.
     */
    public void copyRowRegionFromTemplate(int firstTemplateRownum, int lastTemplateRownum, int rownum) {
        if (firstTemplateRownum > lastTemplateRownum) {
            throw new IllegalArgumentException("First line of row region to copy cannot be after last ("
                    + firstTemplateRownum + " - " + lastTemplateRownum + ")");
        }
        // traverse specified range of rows from template sheet
        for (int templateRownum = firstTemplateRownum; templateRownum <= lastTemplateRownum; templateRownum++) {
            copyRowFromTemplate(templateRownum, rownum + templateRownum - firstTemplateRownum);
        }
    }
...
//    public CellStyle setColumnStyle(int column, CellStyle style);
    ...
    {
        for(RXSSFRow row : _rows.subMap(fromRow, toRow + 1).values()){
    ...
    private void collapseRow(int rowIndex) {
        ...
        if(row == null) {
            ...
            RXSSFRow lastRowObj = getRow(lastRow);
            if (lastRowObj != null) {
                lastRowObj.setCollapsed(true);
            } else {
                RXSSFRow newRow = createRow(lastRow);
    ...
    private int writeHidden(RXSSFRow xRow, int rowIndex) {
        int level = xRow.getOutlineLevel();
        RXSSFRow currRow = getRow(rowIndex);
     ...
     *  the RXSSFSheet using {@link #trackColumnForAutoSizing(int)} or {@link #trackAllColumnsForAutoSizing()}.
     ...
     */
    @Override
    public RXSSFDrawing createDrawingPatriarch()
    {
        return new RXSSFDrawing(getWorkbook(), _sh.createDrawingPatriarch());
    }
     ...
     */
    @Override
    public RXSSFWorkbook getWorkbook()
    {
        return _workbook;
    }
...
//end of interface implementation
    ...
    private void flushOneRow() throws IOException
    {
        ...
        if (firstRowNum!=null) {
            ...
            RXSSFRow row = _rows.get(firstRowNum);
    ...
    public void changeRowNum(RXSSFRow row, int newRowNum)
    {
    ...
    public int getRowNum(RXSSFRow row)
    {
        for (Map.Entry<Integer, RXSSFRow> entry : _rows.entrySet()) {
            if (entry.getValue() == row) {
