...
/**
 * Streaming version of XSSFCell.
 * Unlike regular XSSFCell, this version does not support pre-calculated formula values - it is not able to precalculate
 * values safely, thus precalculated values are striped when copying cells from template. If such support is needed
 * in future, it might be taken from SXSSFCell or XSSFCell implementations in POI project
 */
public class RXSSFCell implements Cell {
    private static final POILogger logger = POILogFactory.getLogger(RXSSFCell.class);

    private final RXSSFRow _row;
    private Value _value;
    private CellStyle _style;
    private Property _firstProperty;
    
    public RXSSFCell(RXSSFRow row, CellType cellType)
    {
        _row=row;
        setType(cellType);
    }
...
//start of interface implementation
     ...
     * @return {@link CellType#NUMERIC} - cached values are not supported in RXSSFCell
     */
    @Override
    ...
    {
        if(_style == null){
            RXSSFWorkbook wb = (RXSSFWorkbook)getRow().getSheet().getWorkbook();
...
//end of interface implementation
    ...
    // not needed - given that we do not support precalculated values, we do not care about formula types...
    @NotImplemented
...
//We cannot introduce a new type CellType.RICH_TEXT because the types are public so we have to make rich text as a type of string
    ...
    static class FormulaValue implements Value
    {
        String _value;
        public CellType getType()
        {
    ...
    /**
     * Copy all relevant properties from template cell
     *
     * @param templateCell is cell from template workbook acting as template for this cell
     */
    public void copyFromTemplate(Cell templateCell) {
        Objects.requireNonNull(templateCell);
        setCellType(templateCell.getCellType());
        switch (getCellType()) {
            case FORMULA:
                // TODO: need to transform cell references
                setCellFormula(templateCell.getCellFormula());
                break;
            case STRING:
                setCellValue(templateCell.getStringCellValue());
                break;
            case NUMERIC:
                setCellValue(templateCell.getNumericCellValue());
                break;
            case BOOLEAN:
                setCellValue(templateCell.getBooleanCellValue());
                break;
            case BLANK:
                // blank value = no value
                break;
            case ERROR:
                setCellErrorValue(templateCell.getErrorCellValue());
             default:
                 throw new RuntimeException("Illegal cell format in template " + getCellType());
        }
        if (templateCell.getCellComment() != null) {
            setCellComment(templateCell.getCellComment());
        }
        if (templateCell.getHyperlink() != null) {
            setHyperlink(templateCell.getHyperlink());
        }
    }
}
