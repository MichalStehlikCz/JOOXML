...
public class RXSSFRow implements Row, RepRow, Comparable<RXSSFRow>
{
    ...
    private final SortedMap<Integer, RXSSFCell> _cells = new TreeMap<>();
    ...
    public RXSSFRow(RXSSFSheet sheet)
    {
        _sheet=sheet;
    }
...
//begin of interface implementation
     ...
     */
    @Override
    public RXSSFCell createCell(int column)
    {
     ...
     */
    @Override
    public RXSSFCell createCell(int column, CellType type)
    {
        checkBounds(column);
        RXSSFCell cell = new RXSSFCell(this, type);
    ...
    {
        int index = getCellIndex((RXSSFCell) cell);
        _cells.remove(index);
    }
     ...
     */
    /*package*/ int getCellIndex(RXSSFCell cell)
    {
        for (Entry<Integer, RXSSFCell> entry : _cells.entrySet()) {
     ...
     */
    @Override
    public RXSSFCell getCell(int cellnum) {
     ...
     */
    @Override
    public RXSSFCell getCell(int cellnum, MissingCellPolicy policy)
    {
        checkBounds(cellnum);

        final RXSSFCell cell = _cells.get(cellnum);
        switch (policy) {
    ...
    /**
     * Compares two <code>RXSSFRow</code> objects.  Two rows are equal if they belong to the same worksheet and
     * their row indexes are equal.
     *
     * @param   other   the <code>RXSSFRow</code> to be compared.
     * @return  <ul>
     *      <li>
     *      the value <code>0</code> if the row number of this <code>RXSSFRow</code> is
     *      equal to the row number of the argument <code>RXSSFRow</code>
     *      </li>
     *      <li>
     *      a value less than <code>0</code> if the row number of this this <code>RXSSFRow</code> is
     *      numerically less than the row number of the argument <code>RXSSFRow</code>
     *      </li>
     *      <li>
     *      a value greater than <code>0</code> if the row number of this this <code>RXSSFRow</code> is
     *      numerically greater than the row number of the argument <code>RXSSFRow</code>
     *      </li>
     *      </ul>
     * @throws IllegalArgumentException if the argument row belongs to a different worksheet
     */
    @Override
    public int compareTo(RXSSFRow other) {
    ...
    {
        if (!(obj instanceof RXSSFRow))
        {
            return false;
        }
        RXSSFRow other = (RXSSFRow) obj;
