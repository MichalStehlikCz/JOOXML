 ...
 *
 * RXSSFWorkbook defaults to using inline strings instead of a shared strings
 ...
 */
public class RXSSFWorkbook implements Workbook, RepWorkbook {
    /**
     * Specifies how many rows can be accessed at most via {@link RXSSFSheet#getRow}.
     * When a new node is created via {@link RXSSFSheet#createRow} and the total number
     * of unflushed records would exceed the specified value, then the
     * row with the lowest index value is flushed and cannot be accessed
     * via {@link RXSSFSheet#getRow} anymore.
     */
    public static final int DEFAULT_WINDOW_SIZE = 100;
    private static final POILogger logger = POILogFactory.getLogger(RXSSFWorkbook.class);

    private final XSSFWorkbook _wb;

    private final Map<RXSSFSheet,XSSFSheet> _sxFromXHash = new HashMap<>();
    private final Map<XSSFSheet, RXSSFSheet> _xFromSxHash = new HashMap<>();
    ...
    XSSFSheet getXSSFSheet(RXSSFSheet sheet)
    {
        return _sxFromXHash.get(sheet);
    }

    RXSSFSheet getSXSSFSheet(XSSFSheet sheet)
    {
        return _xFromSxHash.get(sheet);
    }

    void registerSheetMapping(RXSSFSheet sxSheet, XSSFSheet xSheet)
    {
...
//start of interface implementation
    ...
    {
        return _wb.getSheetIndex(getXSSFSheet((RXSSFSheet)sheet));
    }
     ...
     */
    @Override
    public RXSSFSheet createSheet()
    {
        return createAndRegisterSXSSFSheet(_wb.createSheet());
    }

    RXSSFSheet createAndRegisterSXSSFSheet(XSSFSheet xSheet)
    {
        final RXSSFSheet sxSheet;
        try
        {
            sxSheet=new RXSSFSheet(this,xSheet);
        }
     ...
     */
    @Override
    public RXSSFSheet createSheet(String sheetname)
    {
        return createAndRegisterSXSSFSheet(_wb.createSheet(sheetname));
    }

    /**
     * <i>Not implemented for RXSSFWorkbook</i>
     *
     ...
     */
    @Override
    public RXSSFSheet getSheetAt(int index)
    {
     ...
     */
    @Override
    public RXSSFSheet getSheet(String name)
    {
    ...
    public void close() throws IOException {
        ...
        for (RXSSFSheet sheet : _xFromSxHash.values())
        {
            try {
    ...
    public boolean dispose()
    {
        ...
        for (RXSSFSheet sheet : _sxFromXHash.keySet())
        {
            try {
    ...
    public CreationHelper getCreationHelper() {
        return new RXSSFCreationHelper(this);
    }
